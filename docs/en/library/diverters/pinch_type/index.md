...
A single manual/automatic decision (`manual_mode`/`manual`/`auto`, resolved into `desired_route_B`: FALSE = route A, TRUE = route B) determines which valve opens; the other is always commanded closed — the two instances never arbitrate independently. See [Pinch Valve](../../valves/pinch/index.md) for sub-valve detail.
...
|----|----------------------------|
| [`DIV-E01`](../index.md#diverter-alarms) | Current stable state (`ROUTE_A`/`ROUTE_B`) not confirmed by `XVA.STATUS.is_open`/`XVB.STATUS.is_open` |

A fault on `XVA` or `XVB` feeds into `internal_error` (drives `FAULT`) but doesn't raise its own ID at this level — see [Pinch Valve alarms](../../valves/pinch/index.md#alarms).
