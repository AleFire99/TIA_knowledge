...
| `LC-E01` | Stallo carico | `ALARMS.loading_timeout` — nessun progresso di peso per `loading_timeout`; porta `Loading` in ERROR | Celle di Carico |
| `LC-E02` | Stallo scarico | `ALARMS.unloading_timeout` — nessun progresso di peso per `unloading_timeout`; porta `Unloading` in ERROR | Celle di Carico |
