...
|---------|------|-------------|
| `dat_IN.Status_register.weight_error` | Bool | Copiato in `scale.IN.scale_error` |
| `dat_IN.net_weight` | DWord | Peso netto grezzo, convertito in `scale.IN.current_weight` |
| `dat_IN.decimals` | Word | Numero di decimali del trasmettitore (0–4); seleziona il fattore di scala |
| `dat_OUT.Command_register` | Word | Scritto a `16#4` mentre `scale.CMD.tare_request` è TRUE, altrimenti `0` |
...
Dei campi elencati sopra, solo `Status_register.weight_error`, `net_weight`, `decimals` (in ingresso) e `Command_register` (in uscita) sono letti/scritti da `Pavone_DAT_1400` oggi — vedere la tabella "Segnali di controllo"; il resto è margine del trasmettitore non ancora cablato in logica.
