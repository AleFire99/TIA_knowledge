# Nolvac — Ciclo a Tempo

## Panoramica

**Livello 3 — composito.** `Nolvac — Ciclo a Tempo` è l'unità di convogliamento pneumatico a ciclo aspirazione/pulizia che avvia e conclude ogni fase esclusivamente per durata configurata (`SETTING.suction_time`/`cleaning_time`), senza alcuna retroazione di livello sul materiale convogliato. Incorpora una Valvola a Farfalla SS (Livello 2) e due Elettrovalvole (Livello 1). `XY03` attiva il percorso di aspirazione per convogliare il materiale; `XV01` e `XY02` agiscono in combinazione durante la fase di pulizia per rigenerare il filtro interno.
