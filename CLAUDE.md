...
- **YAML-driven FSM generator.** Mermaid `stateDiagram-v2` is hand-written today; a script
  that renders it (plus the guard-formula code block, state table, Timer table, and Azioni
  di ingresso table) from a YAML source of truth would keep the diagram and the formula from
  drifting apart. The schema for that source of truth is now defined — see "FSM YAML Schema"
  below (`schema/fsm.schema.json`) — but the generator script itself, and backfilling real
  per-device yaml files, are still deferred. Scoped as a follow-up once the template itself
  proved out on real content — it now has, across all 15 pages, so schema definition was
  next in line and is done; the generator is next after that.
...
Schema for the future YAML-driven FSM generator (see the deferred item above) — defines
states/transitions/guards/timers/entry-actions/notes for one FSM, sourced from the
controlling FB's `.s7dcl`. Schema only exists today; no generator consumes it yet and no
per-device files have been backfilled.
