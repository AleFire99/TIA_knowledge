...
- **Backfilling real per-device `.fsm.yaml` files.** The schema (`schema/fsm.schema.json`)
  and the render script (`scripts/render_fsm.py`, see "FSM YAML Schema" below) both exist and
  are validated against a worked example, but no real device has a `.fsm.yaml` yet — Mermaid
  `stateDiagram-v2` blocks in `docs/` are still hand-written today. The render script only
  produces an English draft skeleton (mermaid + guard-formula block + tables); a human or
  Claude Code doc pass still has to write the yaml per device (from `raw/`'s `.s7dcl`) and
  localize the rendered draft into final IT + EN prose — that authoring/localization loop is
  what's left, not the schema or the renderer.
...
controlling FB's `.s7dcl`. Rendered into a draft doc-section skeleton by
`scripts/render_fsm.py` (see deferred item above — no real per-device `.fsm.yaml` exists yet,
only `schema/fsm.example.yaml`):
